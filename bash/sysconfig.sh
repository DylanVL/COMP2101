...
function error-message {
	if ! $? -ne 0; then
		echo "ERROR! Command error found. Consult the sacred texts in error.log"
		error-message 2> >(logger -t $(basename $0) -i -p error.log)
	fi
}
...
function error-exit {
	if ! $? -ne 0; then
		echo "ERROR! Exit Status: $?"2>/dev/null
		exit 1 
	else
		echo "Systems are a go!"
	fi
}
#This function displays help information if the user asks for it on the command line or gives us a bad command line
function displayhelp {
	echo "Usage: [-h | --help ] [--host] [--domain] [--ipconfig] [--os] [--cpu] [--memory] [--disk] [--printer]"
}
