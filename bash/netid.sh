...
#case statement to process the command line
	case "$1" in
		-h | --help )
			echo "Usage: $0 [-h |--help | -v | --verbose] [interfacename]"
			exit
			;;
		-v | --verbose )
			verbose=yes
...
#An array for pulled unique interfaces
intList=($(ip r | grep -v default | awk '{print $3}' | uniq))

#intNum=$(echo ${#intList[@]})

#Using a for loop to print out a interface report for each interface listed
#Taking the existing hard coded script below and plugging it in the case area.

for interface in $intList; do
	echo "Here are your interfaces: $intList"
	interface=$intList
	#Verbose stuff if the user would like to see them
	...
	ipv4_address=$(ip a s $interface|grep -wv 'lo'|awk -F '[/ ]+' '/inet /{print $3}')
